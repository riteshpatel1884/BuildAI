...
export function NotesSidebar() {
  ...
  // Basics starts open since it's the only group with content so far.
  const [openGroups, setOpenGroups] = useState({ basics: true });
  ...
  return (
    <nav
      aria-label="Python notes"
      className="w-full shrink-0 text-sm sm:w-64"
    >
          ...
          return (
            <div key={group.slug} className="mb-1">
              <button
                ...
                className="flex w-full items-center justify-between rounded-lg px-2 py-2 text-left font-medium text-zinc-500 hover:text-zinc-900 dark:text-zinc-400 dark:hover:text-white transition-colors"
                ...
                <div className="ml-2 flex flex-col gap-0.5 border-l border-zinc-200 pl-3 dark:border-white/10">
                  {hasNotes ? (
                    group.notes.map((note) => {
                      ...
                      const available = Boolean(note.cells);
                      return (
                        <Link
                          key={note.slug}
                          href={`/python/${note.slug}`}
                          className={[
                    ...
                    <span className="px-2.5 py-1.5 text-zinc-400 dark:text-zinc-600">
                      Coming soon
                    </span>
                  )}
                </div>
